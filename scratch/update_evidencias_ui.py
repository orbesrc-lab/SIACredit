...
# Find all table definitions that have <th>Acción</th> but not Evidencia
...
for table_id in set(all_tables):
    ...
    if table_html_match:
        ...
        if '<th>Acción</th>' in table_html and 'Evidencia' not in table_html:
            ...
            # Update addRow function body to insert the <td> for adjunto
            ...
            # Note: We will use the generic adjunto_html placeholder, and then replace ALL placeholders with SVG later.
            adjunto_html = r"""
        <td>
            ${data.adjunto ? 
        ...
        \2"""
            ...
            # Also modify extract function to pull the adjunto
            # It pushes an object into an array. We search for the push for this specific table.
...
# 3. Replace the text buttons with Graphical Icons for ALL tables (even previously modified ones)
# Text to replace:
# "⬇️ Ver" -> ICON_VIEW
# "✕" -> ICON_DEL
# "Subir..." -> ICON_UPLOAD
# Wait, let's just replace the exact substrings I used in the previous script.

# Previous script used:
# <a ...>⬇️ Ver</a>
...
# Now replace the placeholders ICON_... that I just generated for the new tables
...
# 4. Modify the javascript inside uploadGridEvidence to also use icons when generating HTML dynamically
...
print("Updates applied to evidencias.html")
