...
eval_target = """<div id="unitEvaluationModal" style="display:none; position:fixed; inset:0; background:rgba(0,0,0,0.5); z-index:1001; align-items:center; justify-content:center;">
        <div style="background:white; border-radius:16px; padding:30px; width:460px; max-width:90vw; box-shadow:0 20px 60px rgba(0,0,0,0.3);">
            <h3 style="margin-bottom:20px; font-size:1.2rem;">Crear Evaluacin (Escala CNA 1.0 - 5.0)</h3>
            <input type="hidden" id="evaluationUnitIdx">
            <div class="form-group">
                <label>Ttulo de la Evaluacin / Examen</label>
            ...
            <div style="display:flex; gap:10px; justify-content:flex-end; margin-top:25px;">
                ...
                <button class="btn-primary" onclick="submitAddUnitEvaluation()">Crear Evaluacin</button>
            </div>
        </div>
    </div>"""

eval_target = eval_target.replace('', 'ó') # Fix encoding if any, but let's just use find/substring
...
# 3. Inject JS Functions (at the end before window.onload = function)
js_injection = """
...
content = content.replace("window.onload = function() {", js_injection + "\n        window.onload = function() {")
