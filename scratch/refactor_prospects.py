...
for i, line in enumerate(lines):
    if "# --- CRM / PROSPECTOS RUTAS ---" in line:
        start_idx = i
        break
...
if start_idx != -1:
    ...
    bp_code = "from flask import Blueprint, jsonify, request, render_template\n"
    bp_code += "from utils.db import supabase, get_active_inst_id\n"
    bp_code += "import traceback\n\n"
    
    bp_code += "prospects_bp = Blueprint('prospects', __name__)\n\n"
    
    for line in extract_lines:
        line = line.replace("@app.route", "@prospects_bp.route")
        bp_code += line
        
    with open(r'c:\SIAC\routes\prospects.py', 'w', encoding='utf-8') as f:
        f.write(bp_code)
    ...
    print(f"Extracted {len(extract_lines)} lines for prospects blueprint.")
else:
