                            ...
                            rArr.forEach((r, rIdx) => {
                                ...
                                resHtml += `
                                    <div style="border: 1px solid var(--border-color); border-radius: 12px; overflow: hidden; background: #fff;">
                                        <div onclick="toggleTopicAccordion('${resId}')" style="padding: 12px 18px; font-weight: 600; font-size: 0.88rem; cursor: pointer; display: flex; justify-content: space-between; align-items: center; background: #f8fafc; color: var(--text-main); transition: background 0.2s;">
                                            <span><strong>${typeIcon}:</strong> ${escapeHtml(r.name)}</span>
                                            <div style="display:flex; gap:10px; align-items:center;">
                            ...
                            unit.evaluations.forEach(ev => {
                                evalHtml += `
                                    <div class="evaluation-item" style="margin-top:5px;">
                                        ...
                                        <button class="btn-primary" style="padding:8px 16px; font-size:0.8rem; background: var(--primary-gradient); border:none; color:white; border-radius:10px;" onclick="takeQuiz(${uIdx}, '${ev.id}')">Presentar Examen</button>
                                    </div>`;
                            });
                        }
        ...
        async function processActivitySubmission() {
            ...
            const answer = document.getElementById('subContent').value;
            
            if (!answer || answer.trim() === '') {
                alert("Debes escribir una respuesta o adjuntar un enlace de entrega.");
                return;
            }
        ...
        function presentQuiz(uIdx, evIdx) {
            ...
            let questions = (ev.questions && ev.questions.length > 0) ? ev.questions : cnaQuestionBank.map(cq => ({
                text: cq.q,
                type: 'single',
                options: cq.options,
        ...
        async function loadCourseSubmissions(courseId) {
            ...
            try {
                const resp = await fetch('/api/submissions?course_id=' + courseId + '&inst_id=' + getInstId() + '&program_id=' + getProgramId());
        ...
        async function processGrading() {
            ...
            try {
                const courseId = document.getElementById('editCourseId').value;
                const checkResp = await fetch('/api/submissions?course_id=' + courseId + '&inst_id=' + getInstId() + '&program_id=' + getProgramId());
