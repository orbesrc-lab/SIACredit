...
// User Management Functions for SIACredit - V2
console.log("SIACredit: users_v2.js loaded");

window.loadUsersFromAPI = async function() {
    console.log("loadUsersFromAPI called");
    ...
    try {
        ...
        if (!Array.isArray(users) || users.length === 0) {
            container.innerHTML = '<div style="padding:20px; text-align:center; color:var(--text-muted);">No hay usuarios registrados para esta institución/programa.</div>';
            return;
        }
                ...
                <div class="user-item">
                    ...
                    <div style="display:flex; align-items:center; gap:8px; flex-wrap: wrap; justify-content: flex-end;">
                        ...
                        ${!isCurrentUser && !isPending ? 
                            `<button onclick="window.resetUserPass(${u.id}, '${u.email}')" class="btn-ghost" style="font-size:0.7rem; color:var(--text-muted); border: 1px solid #ddd; padding: 3px 8px; border-radius: 4px;">Resetear Clave</button>` : ''}
                        
                        ${!isCurrentUser && canManage ? 
                            `<button onclick="window.deleteUserFromList(${u.id}, '${u.email}')" class="btn-ghost" style="font-size:0.7rem; color:#ef4444; border: 1px solid #ef4444; padding: 3px 8px; border-radius: 4px;">Eliminar</button>` : ''}
                    </div>
                </div>`;
        }).join('');
    } catch(e) {
        console.error("Error loading users:", e);
        container.innerHTML = '<div style="padding:20px; color:#ef4444;">Error al cargar usuarios: ' + e.message + '</div>';
    }
};

window.activateUser = async function(userId, email, currentRole) {
    console.log("activateUser called", userId, email);
...
window.changeUserRole = async function(userId, email, currentRole) {
    console.log("changeUserRole called", userId, email);
    ...
    if (!['lider', 'operativo', 'inst_admin'].includes(cleanRole)) {
...
window.resetUserPass = async function(userId, email) {
    console.log("resetUserPass called", userId, email);
    if (!confirm(`¿Está seguro de resetear la contraseña de ${email}?\n\nSe generará una contraseña temporal.`)) return;
    try {
...
window.deleteUserFromList = async function(userId, email) {
    console.log("deleteUserFromList called", userId, email);
    if (!confirm(`⚠️ ¿ELIMINAR definitivamente al usuario ${email}?\n\nEsta acción no se puede deshacer.`)) return;
    try {
